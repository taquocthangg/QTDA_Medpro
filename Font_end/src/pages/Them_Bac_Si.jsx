...
const ThemBacSi = () => {
  ...
  const Delete_chuyen_khoa = async (idUser) => {
      try {
        ...
        const response = await axios.delete(`https://medprov2.onrender.com/api/v1/auth/xoaUser/` + idUser);
         console.log(idUser)
  ...
  useEffect(() => {
    axios.get('https://medprov2.onrender.com/api/v1/auth/chuyenkhoa/'+ getId)
      .then(response => {
  ...
  useEffect(() => {
    ...
    if (getId_Doctor) {
      setLoading(true);
      axios.get('https://medprov2.onrender.com/api/v1/auth/getBacSiByChuyenKhoa/' + getId_Doctor)
        .then(response => {
